...
public class IpAddress {
    ...
    private static String numericToTextFormatV4(byte[] src) {
        return (src[0] & 0xff) + SymbolConstants.PERIOD + (src[1] & 0xff) + SymbolConstants.PERIOD + (src[2] & 0xff) + SymbolConstants.PERIOD + (src[3] & 0xff);
    }
    ...
    private static String numericToTextFormatV6(byte[] src) {
        ...
        for (int i = 0; i < (INADDR16SZ / INT16SZ); i++) {
            ...
            if (i < (INADDR16SZ / INT16SZ) - 1) {
                sb.append(SymbolConstants.COLON);
            }
        }
        return sb.toString();
    }
