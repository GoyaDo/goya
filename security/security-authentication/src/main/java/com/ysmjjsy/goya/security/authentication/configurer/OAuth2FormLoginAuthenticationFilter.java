...
import com.ysmjjsy.goya.security.authentication.provider.OAuth2FormLoginAuthenticationToken;
...
public class OAuth2FormLoginAuthenticationFilter extends UsernamePasswordAuthenticationFilter {
    ...
    private OAuth2FormLoginAuthenticationToken getAuthenticationToken(
        ...
        if (StringUtils.isNotBlank(username) && StringUtils.isNotBlank(password)) {
            username = httpCryptoProcessor.decrypt(requestId, username);
            password = httpCryptoProcessor.decrypt(requestId, password);
            log.debug("[Goya] |- Decrypt Username is : [{}], Password is : [{}]", username, password);
        }
