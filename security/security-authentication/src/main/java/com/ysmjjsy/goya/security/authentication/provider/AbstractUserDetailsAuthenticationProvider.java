...
import com.ysmjjsy.goya.security.authentication.utils.OAuth2AuthenticationProviderUtils;
...
public abstract class AbstractUserDetailsAuthenticationProvider extends AbstractAuthenticationProvider {
    ...
    public AbstractUserDetailsAuthenticationProvider(OAuth2AuthorizationService authorizationService, UserDetailsService userDetailsService, IdentityProperties identityProperties) {
    ...
    private Authentication authenticateUserDetails(Map<String, Object> additionalParameters, String registeredClientId) throws AuthenticationException {
        ...
        if (!user.isAccountNonLocked()) {
            log.debug("[Goya] |- Failed to authenticate since user account is locked");
            throw new LockedException(messages.getMessage("AbstractUserDetailsAuthenticationProvider.locked", "User account is locked"));
        }
        if (!user.isEnabled()) {
            log.debug("[Goya] |- Failed to authenticate since user account is disabled");
            throw new DisabledException(messages.getMessage("AbstractUserDetailsAuthenticationProvider.disabled", "User is disabled"));
        }
        if (!user.isAccountNonExpired()) {
            log.debug("[Goya] |- Failed to authenticate since user account has expired");
        ...
        if (!user.isCredentialsNonExpired()) {
            log.debug("[Goya] |- Failed to authenticate since user account credentials have expired");
        ...
        if (identityProperties.getSignInEndpointLimited().getEnabled() && !identityProperties.getSignInKickOutLimited().getEnabled()) {
            if (authorizationService instanceof EnhanceOAuth2AuthorizationService enhanceOAuth2AuthorizationService) {
                ...
                if (count >= identityProperties.getSignInEndpointLimited().getMaximum()) {
            ...
            if (authorizationService instanceof EnhanceOAuth2AuthorizationService enhanceOAuth2AuthorizationService) {
                ...
                if (CollectionUtils.isNotEmpty(authorizations)) {
                    authorizations.forEach(authorization -> {
                        ...
                        log.debug("[Goya] |- Sign in user [{}] with token id [{}] will be kicked out.", user.getUsername(), authorization.getId());
    ...
    protected Authentication getUsernamePasswordAuthentication(Map<String, Object> additionalParameters, String registeredClientId) throws AuthenticationException {
        ...
        try {
            ...
            OAuth2EndpointUtils.throwError(
                    SecurityConstants.BAD_CREDENTIALS,
                    bce.getMessage(),
                    OAuth2EndpointUtils.ACCESS_TOKEN_REQUEST_ERROR_URI);
